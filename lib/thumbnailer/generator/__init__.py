...
def generate():
    ...
    bpy.context.scene.cycles.samples = int(sys.argv[-2])
    bpy.context.scene.render.resolution_percentage = int(sys.argv[-1])


    if sys.argv[-3] == "full":
        bpy.context.scene.render.filepath = "C:\\Temp\\" + objFilename.replace(".obj", "_full.jpg").split("\\")[-1]
        bpy.ops.render.render(write_still=True)

    elif sys.argv[-3] == "quad":
        for i in range(0, 360, 90):
            bpy.context.object.rotation_euler[2] = i * 0.0174532925
            bpy.context.scene.render.filepath = "C:\\Temp\\" + objFilename.replace(".obj", "_" + str(i).zfill(3) + ".jpg").split("\\")[-1]
            bpy.ops.render.render(write_still=True)

    elif sys.argv[-3] == "turn":
        for i in range(0, 360, 15):
            bpy.context.object.rotation_euler[2] = i * 0.0174532925
            bpy.context.scene.render.filepath = "C:\\Temp\\" + objFilename.replace(".obj", "_" + str(int(i/15)).zfill(2) + ".jpg").split("\\")[-1]
            bpy.ops.render.render(write_still=True)
