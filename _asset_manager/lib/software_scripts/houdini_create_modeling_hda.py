...
shading_hda_path = sys.argv[-3].replace("\\", "/")
...
main_hda = sys.argv[-5].replace("\\", "/") + "/_NEF/main_hda.hdanc"
...
# Set parameters
file_highres_node = hou.node("/obj/" + asset_name.replace("-", "_") + "/layout/high-res_obj_from_modeling")
file_lowres_node = hou.node("/obj/" + asset_name.replace("-", "_") + "/layout/low-res_obj_from_modeling")
rop_geo_high = hou.node("/obj/" + asset_name.replace("-", "_") + "/layout/save_low_res_geometry")
rop_geo_low = hou.node("/obj/" + asset_name.replace("-", "_") + "/layout/save_high_res_geometry")
...
# MATERIAL NODE
# Shopnet / Copnet
shopnet_node = hou.node("/obj/" + asset_name.replace("-", "_")).createNode("shopnet")
...
material_node = hou.node("/obj/" + asset_name.replace("-", "_")).collapseIntoSubnet([shopnet_node])
